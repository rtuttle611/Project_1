...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gevents = __webpack_require__(5);
	...
	var onSignIn = function onSignIn(event) {
	  event.preventDefault();
	  var data = getFormFields(this);
	  api.signIn(data).then(ui.signInSuccess).catch(function (e) {
	    console.log("====ERROR=====", e);
	    return e;
	  });
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gapi = __webpack_require__(6);
	...
	var onUpdateGame = function onUpdateGame(index, value, over) {
	  ...
	  gapi.updateGame(data).then(gui.success).catch(gui.failure);
	};
	...
	var onCellClick = function onCellClick(e) {
	  ...
	  if (!$(this).data('data-mark')) {
	    ...
	    var clickedCellIndex = $('.cell-content').index($(this));
	    ...
	    var clickedCellValue = $(this).data('data-mark');
	    var currentState = gameState.stateUpdate();
	    isOver = false;
	...
	var playerTurn = function playerTurn(spot) {
	  gameArray = [];
	  $(this).text(currentPlayer);

	  $('#spots li').each(function (i, e) {
	...
	var allThree = function allThree(player, spot1, spot2, spot3) {
	  return spot1 === player && spot2 === player && spot3 === player;
	};
	var winningRow = function winningRow(player) {
	  return allThree(player, gameArray[0], gameArray[1], gameArray[2]) || allThree(player, gameArray[3], gameArray[4], gameArray[5]) || allThree(player, gameArray[6], gameArray[7], gameArray[8]);
	};
	var winningCol = function winningCol(player) {
	  return allThree(player, gameArray[0], gameArray[3], gameArray[6]) || allThree(player, gameArray[1], gameArray[4], gameArray[7]) || allThree(player, gameArray[2], gameArray[5], gameArray[8]);
	};
	var winnningDiag = function winnningDiag(player) {
	  return allThree(player, gameArray[0], gameArray[4], gameArray[8]) || allThree(player, gameArray[2], gameArray[4], gameArray[6]);
	};

	var onCreateGame = function onCreateGame() {
	  gapi.createGame().then(gui.createGameSuccess).catch(gui.failure);
	};
	...
	var onGameInfo = function onGameInfo(e) {
	  e.preventDefault();
	  gapi.gameInfo().then(gui.gameInfoSuccess).catch(gui.failure);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gameInfoSuccess = function gameInfoSuccess(data) {
	...
	module.exports = {
	  failure: failure,
	  success: success,
	  gameInfoSuccess: gameInfoSuccess,
	...
	*/
	...
	var setAPIOrigin = function setAPIOrigin(location, config) {
	  ...
	  if (search.environment === 'development' || location.hostname === 'localhost' && search.environment !== 'production') {
	    if (!(config.apiOrigin = config.apiOrigin.development)) {
